...
async function main() {
  try {
    ...
    // Step 6: Create INFRA_PAT secret on the app repo
    console.log('Step 6: Configurando secreto INFRA_PAT en el repositorio...');
    const githubToken = process.env.GITHUB_TOKEN;
    if (!githubToken) {
      throw new Error('GITHUB_TOKEN environment variable is not set');
    }
    await githubService.createRepoSecret(
      jobData.repoOwner,
      repoSlug,
      'INFRA_PAT',
      githubToken,
    ...
    console.log('Step 7: Creando manifiestos K8s en infra-live...');
    const infraFiles = templateService.generateInfraFiles(variables);
    console.log(`✓ Generados ${Object.keys(infraFiles).length} manifiestos K8s`);
    Object.keys(infraFiles).forEach(f => console.log(`   - ${f}`));

    const infraTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'devops-infra-'));
    console.log(`📁 Directorio temporal infra: ${infraTmpDir}`);

    try {
      const infraCloneUrl = `https://${githubToken}@github.com/${jobData.repoOwner}/infra-live.git`;
      console.log(`📥 Clonando infra-live...`);
      execSync(`git clone ${infraCloneUrl} ${infraTmpDir}`, { stdio: 'inherit' });
      console.log('✓ infra-live clonado');

      // Configure git
      execSync('git config user.name "DevOps Agent R2D2"', { cwd: infraTmpDir });
      execSync('git config user.email "devops-agent@example.com"', { cwd: infraTmpDir });

      // Create apps/<name>/ directory
      const appDir = path.join(infraTmpDir, 'apps', jobData.name);
      fs.mkdirSync(appDir, { recursive: true });
      console.log(`📁 Directorio creado: apps/${jobData.name}/`);

      // Write manifest files
      for (const [fileName, content] of Object.entries(infraFiles)) {
        fs.writeFileSync(path.join(appDir, fileName), content);
        console.log(`   ✓ apps/${jobData.name}/${fileName}`);
      }

      // Commit and push
      console.log('📤 Haciendo commit y push a infra-live...');
      execSync('git add .', { cwd: infraTmpDir });
      
      // Check if there are changes to commit
      try {
        execSync('git diff --staged --quiet', { cwd: infraTmpDir });
        console.log('ℹ️  No hay cambios en infra-live (manifiestos ya existían)');
      } catch {
        // diff --quiet exits with 1 if there are changes — that's what we expect
        execSync(
          `git commit -m "feat(${jobData.name}): add Kubernetes manifests\n\nGenerated by DevOps Agent R2D2"`,
          { cwd: infraTmpDir, stdio: 'inherit' },
        );
        execSync('git push origin main', { cwd: infraTmpDir, stdio: 'inherit' });
        console.log('✓ Manifiestos publicados en infra-live');
      }
      console.log('');

      // Clean up
      fs.rmSync(infraTmpDir, { recursive: true, force: true });
      console.log('✓ Directorio temporal infra eliminado');
    } catch (error) {
      if (fs.existsSync(infraTmpDir)) {
        fs.rmSync(infraTmpDir, { recursive: true, force: true });
      }
      throw error;
    }
    ...
    try {
      ...
      if (error?.statusCode === 409 || error?.body?.reason === 'AlreadyExists') {
        console.log(`ℹ️  ArgoCD Application "${jobData.name}" ya existía`);
      } else {
        throw error;
      }
    }
    ...
    console.log(`Domain: ${jobData.domain || 'not configured'}`);
    console.log(`Infra-live: https://github.com/${jobData.repoOwner}/infra-live/tree/main/apps/${jobData.name}`);
    console.log(`ArgoCD: Application "${jobData.name}" will sync automatically`);
    console.log('');
